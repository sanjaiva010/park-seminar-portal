...
// ---------- validation helpers ----------
// Letters, spaces, and common name punctuation only (e.g. "Dr. A. Kumar-Rao")
function isLettersOnly(str) {
  return /^[A-Za-z][A-Za-z\s.'-]*$/.test(str.trim());
}
// Letters + numbers + everyday punctuation, for things like function names
function isAlphaNumeric(str) {
  return /^[A-Za-z0-9][A-Za-z0-9\s.,'&()\/-]*$/.test(str.trim());
}
// Exactly 10 digits
function isValidMobile(str) {
  return /^[0-9]{10}$/.test(str.trim());
}
// Restrict a text input to letters/spaces only as the user types
function restrictToLetters(input) {
  input.addEventListener("input", () => {
    input.value = input.value.replace(/[^A-Za-z\s.'-]/g, "");
  });
}
// Restrict a text input to digits only as the user types, capped at a length
function restrictToDigits(input, maxLen) {
  input.addEventListener("input", () => {
    input.value = input.value.replace(/[^0-9]/g, "").slice(0, maxLen || 10);
  });
}
// Show/hide a field-level error message under an input
...
// ---------- live sync across portals ----------
// Subscribes to booking changes so Staff / HoD / Super Admin all stay in sync
// without needing a manual refresh. Requires migration-3.sql to have been run
// (it adds bookings/booking_halls/booking_slots to the Realtime publication).
function subscribeToBookingChanges(onChange) {
  const channel = supabaseClient
    .channel("booking-sync-" + Math.random().toString(36).slice(2))
    .on("postgres_changes", { event: "*", schema: "public", table: "bookings" }, onChange)
    .on("postgres_changes", { event: "*", schema: "public", table: "booking_halls" }, onChange)
    .on("postgres_changes", { event: "*", schema: "public", table: "booking_slots" }, onChange)
    .subscribe();
  return channel;
}
